...
class Solver(object):
    ...
    def train(self,centers):
        ...
        for batch_num, (data, target) in enumerate(self.train_loader):
            ...
            #print(centers[target].size())
            #print(centers[target,:].size())
            #print(centers[target])
            #print(centers[target,:])
            #print(output.size())
            #print(mse(output, centers[target]))
            loss = self.criterion(output, target) + mse(output, centers[target])
            loss.backward()
            self.optimizer.step()
            ...
            for idx in range(data.size()[0]):
              if centers[target[idx]] is False:
                centers[target[idx]] = output[idx].detach()
              else:
                centers[target[idx]] = (1 - param)  * centers[target[idx]] + param * output[idx].detach()
    ...
    def run(self):
        ...
        self.load_model()
        accuracy = 0
        centers = torch.zeros(100, 100).cuda()
        for epoch in range(1, self.epochs + 1):
            self.scheduler.step(epoch)
            print("\n===> epoch: %d/200" % epoch)
            train_result = self.train(centers)

...
class Solver(object):
    ...
    def train(self,centers):
        ...
        for batch_num, (data, index, target) in enumerate(self.train_loader):
            data, target = data.to(self.device), target.to(self.device)
            self.optimizer.zero_grad()
            mse = torch.nn.MSELoss()
            output = self.model(data)
            loss = self.criterion(output, target) + mse(output, centers[index].detach())
            ...
            total += target.size(0)
            param = self.optimizer.param_groups[0]['lr']
            output = self.model(data)
        ...
        #      print(index[idx])
              if centers[index[idx]] is False:
                centers[index[idx]] = output[idx].detach()
              else:
                centers[index[idx]] = centers[index[idx]] + param * (output[idx].detach() - centers[index[idx]])

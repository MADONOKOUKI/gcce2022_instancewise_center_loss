...
def main():
    ...
    parser.add_argument('--N', default=1, type=int, help='num of augmented images')
    parser.add_argument('--M', default=2, type=int, help='num of augmented images')
    parser.add_argument('--mean', default=0, type=int, help='num of augmented images')
...
class Solver(object):
    ...
    def train(self,centers,epoch):
        ...
        for batch_num, (imgs, index, target) in enumerate(self.train_loader):
            ...
            mean /= self.num_imgs
            for idx in range(self.num_imgs):
               if self.config.mean == 1:
                 loss += mse(outputs[idx],mean)

               loss += self.criterion(outputs[idx], target) #+ mse(outputs[idx],mean)     
                  
            loss /= self.num_imgs
            # if self.config.mean == 1: 
            #     loss += self.criterion(mean, target) #+ mse(outputs[idx],mean)

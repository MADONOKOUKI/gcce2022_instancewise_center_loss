...
class Solver(object):
    ...
    def train(self,centers,epoch):
        ...
        for batch_num, (imgs, index, target) in enumerate(self.train_loader):
            ...
            data, target = imgs[0].to(self.device), target.to(self.device)
            ...
            if epoch == 0:
                ...
                ## compute ideal center position
                p = F.softmax(output).detach()
                one_hot_t = torch.from_numpy(to_one_hot_vector(100,target.cpu()).astype(np.float32)).cuda()
                one_hot_t = torch.sum(p * one_hot_t,dim=-1).unsqueeze(-1) 

                p = F.softmax(centers[index]).detach()
                one_hot_t2 = torch.from_numpy(to_one_hot_vector(100,target.cpu()).astype(np.float32)).cuda()
                one_hot_t2 = torch.sum(p * one_hot_t2,dim=-1).unsqueeze(-1) 

#                center = (centers[index] * one_hot_t2 + output * one_hot_t) / (one_hot_t2 + one_hot_t)
                center = (centers[index] + output) / 2

                ## compute loss with constraint
                loss += self.criterion(output, target) + mse(output, center)
                centers[index] = center.detach()
            loss /= self.num_imgs
